...
const TasksAndActivities = () => {
  ...
  const { isOpen: showSponsorRequestModal, onOpen: onSponsorRequestModalOpen, onClose: onSponsorRequestModalClose } = useDisclosure();
  ...
  const fetchCurrentSponsor = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/coach-hierarchy/details`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'X-Coach-Id': user._id || user.id || user.coachId
        }
      });
  ...
  // Fetch my admin requests
  const fetchMyRequests = async () => {
    if (!token || !user) return;
    
    setLoadingRequests(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/coach-hierarchy/my-requests`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'X-Coach-Id': user._id || user.id || user.coachId
        }
  ...
  // Submit sponsor change request
  const submitSponsorRequest = async () => {
    if (!selectedSponsor || !requestForm.reason.trim()) {
      toast({
        title: 'Validation Error',
        description: 'Please select a sponsor and provide a reason',
        status: 'error',
        duration: 3000,
      });
      return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: 'Success',
          description: 'Sponsor change request submitted successfully',
          status: 'success',
          duration: 3000,
        });
        
        onSponsorRequestModalClose();
    ...
    <Box bg="gray.50" minH="100vh" py={6} px={6}>
      <Box maxW="full" mx="auto">
        <VStack spacing={6} align="stretch" w="full">
          ...
          <Tabs defaultIndex={0} index={activeTabIndex} onChange={setActiveTabIndex}>
            <TabList bg="white" borderBottom="2px" borderColor="gray.200" px={6} py={3}>
              ...
              <Tab 
                fontSize="md" 
                fontWeight="600" 
                _selected={{ color: 'orange.600', borderBottomColor: 'orange.600' }}
                color="gray.600"
                pb={3}
              >
                Admin Requests
            ...
            <TabPanels>
              ...
              {/* Admin Requests Tab - Sponsor Change Requests */}
              <TabPanel px={0}>
                <Box mb={4} p={4} bg="linear-gradient(135deg, rgba(251, 146, 60, 0.05) 0%, rgba(245, 158, 11, 0.05) 100%)" borderRadius="md" border="1px" borderColor="orange.200">
                  <HStack spacing={3}>
                    <Box as={FiUser} color="orange.600" boxSize={5} />
                    <VStack align="start" spacing={1}>
                      <Text fontWeight="700" color="orange.800" fontSize="lg">Admin Requests</Text>
                      <Text color="orange.600" fontSize="sm">Submit requests to change your sponsor or update hierarchy information. Admin approval required.</Text>
                    </VStack>
                  </HStack>
                </Box>
                
                <VStack spacing={6} align="stretch">
                  {/* Sponsor Change Request Section */}
                  <Card bg={cardBg} boxShadow="sm" border="1px" borderColor={borderColor} borderRadius="7px">
                    <CardHeader>
                      <HStack justify="space-between" align="center">
                        <VStack align="start" spacing={1}>
                          <Heading size="md" color={textColor}>🔄 Sponsor Change Request</Heading>
                          <Text fontSize="sm" color={mutedTextColor}>
                            Request to change your main sponsor. This requires admin approval.
                          </Text>
                        </VStack>
                        <Button 
                          colorScheme="orange" 
                          size="sm"
                          onClick={() => setShowSponsorRequestModal(true)}
                        >
                          New Request
                        </Button>
                      </HStack>
                    ...
                    <CardBody>
                      ...
                      <Box mb={6} p={4} bg="blue.50" borderRadius="md" border="1px" borderColor="blue.200">
                        <VStack align="start" spacing={2}>
                          <Text fontWeight="600" color="blue.800">Current Sponsor Information</Text>
                          {currentSponsor ? (
                            <HStack spacing={3}>
                              <Avatar name={currentSponsor.name} size="sm" bg="blue.500" />
                              <VStack align="start" spacing={1}>
                                <Text fontWeight="500" color="blue.700">{currentSponsor.name}</Text>
                                <Text fontSize="sm" color="blue.600">{currentSponsor.email}</Text>
                                <Text fontSize="xs" color="blue.500">ID: {currentSponsor.selfCoachId || 'N/A'}</Text>
                              </VStack>
                            </HStack>
                          ) : (
                            <Text color="blue.600">No sponsor assigned</Text>
                          )}
                        </VStack>
                      </Box>
                      ...
                      <VStack align="stretch" spacing={4}>
                        <HStack justify="space-between">
                          <Text fontWeight="600" color={textColor}>My Request History</Text>
                          <Button 
                            variant="ghost" 
                            size="xs" 
                            leftIcon={<FiRefreshCw />}
                        ...
                        {myRequests.length === 0 ? (
                          <Center py={8}>
                            <VStack spacing={3}>
                              <Box as={FiUser} boxSize={12} color="gray.300" />
                              <Text color={mutedTextColor} textAlign="center">
                                No sponsor change requests found
                              </Text>
                              <Text fontSize="sm" color="gray.400">
                                Click "New Request" to submit your first sponsor change request
                              </Text>
                            </VStack>
                          </Center>
                        ) : (
                              ...
                              >
                                <CardBody>
                                  <VStack align="stretch" spacing={3}>
                                    ...
                                    {request.requestedData?.sponsorName && (
                                      <Box p={2} bg="orange.50" borderRadius="md">
                                        <Text fontSize="xs" color="orange.700">
                                          <strong>Requested Sponsor:</strong> {request.requestedData.sponsorName}
                                        </Text>
                                      </Box>
                                    )}
          ...
          {/* Sponsor Change Request Modal */}
          <Modal isOpen={showSponsorRequestModal} onClose={onSponsorRequestModalClose} size="lg">
            <ModalOverlay />
            <ModalContent bg={cardBg} color={textColor}>
              <ModalHeader>
                <HStack spacing={3}>
                  ...
                  <VStack align="start" spacing={0}>
                    <Text fontSize="lg" fontWeight="bold">Sponsor Change Request</Text>
                    <Text fontSize="sm" color="gray.500">Request to change your main sponsor</Text>
                  </VStack>
                </HStack>
              </ModalHeader>
              <ModalCloseButton />
              <ModalBody>
                <VStack spacing={6} align="stretch">
                  ...
                  <Box p={4} bg="blue.50" borderRadius="md" border="1px" borderColor="blue.200">
                    <Text fontWeight="600" color="blue.800" mb={3}>Current Sponsor</Text>
                    {currentSponsor ? (
                      <HStack spacing={3}>
                        <Avatar name={currentSponsor.name} size="sm" bg="blue.500" />
                        <VStack align="start" spacing={1}>
                  ...
                  {/* Sponsor Search */}
                  <FormControl>
                    <FormLabel>Search for New Sponsor</FormLabel>
                    <Input
                      placeholder="Type name, email, or coach ID (min 3 characters)..."
                  ...
                  {sponsorSearchResults.length > 0 && (
                    <Box>
                      <Text fontWeight="600" color={textColor} mb={3}>Search Results</Text>
                      <VStack spacing={2} maxH="200px" overflowY="auto">
                        {sponsorSearchResults.map((sponsor) => (
                          <Card
                            ...
                            borderWidth={selectedSponsor?._id === sponsor._id ? '2px' : '1px'}
                            borderColor={selectedSponsor?._id === sponsor._id ? 'orange.500' : borderColor}
                          >
                            <CardBody p={3}>
                              <HStack spacing={3}>
                                <Avatar name={sponsor.name} size="sm" bg="orange.500" />
                                <VStack align="start" spacing={1} flex={1}>
                                  <Text fontWeight="500" color={textColor}>{sponsor.name}</Text>
                                  <Text fontSize="sm" color={mutedTextColor}>{sponsor.email}</Text>
                                  <HStack spacing={2}>
                                    <Badge colorScheme="blue" size="xs">
                                      {sponsor.type === 'digital' ? 'Digital' : 'External'}
                                    </Badge>
                                    <Text fontSize="xs" color="gray.500">
                                      ID: {sponsor.displayId}
                                    </Text>
                                  </HStack>
                                </VStack>
                                {selectedSponsor?._id === sponsor._id && (
                                  <Box as={FiCheck} color="orange.500" boxSize={5} />
                                )}
                              </HStack>
                            </CardBody>
                          </Card>
                  ...
                  {selectedSponsor && (
                    <Box p={4} bg="orange.50" borderRadius="md" border="1px" borderColor="orange.200">
                      <Text fontWeight="600" color="orange.800" mb={2}>Selected New Sponsor</Text>
                      <HStack spacing={3}>
                        <Avatar name={selectedSponsor.name} size="sm" bg="orange.500" />
                        <VStack align="start" spacing={1}>
                          <Text fontWeight="500" color="orange.700">{selectedSponsor.name}</Text>
                          <Text fontSize="sm" color="orange.600">{selectedSponsor.email}</Text>
                          <Text fontSize="xs" color="orange.500">
                            Type: {selectedSponsor.type === 'digital' ? 'Digital Coach' : 'External Sponsor'}
                          </Text>
                        </VStack>
                      </HStack>
                    </Box>
                  ...
                  <FormControl isRequired>
                    ...
                    <Textarea
                      placeholder="Please explain why you want to change your sponsor..."
                  ...
                  <Alert status="info" borderRadius="md">
                    <AlertIcon />
                    <VStack align="start" spacing={1}>
                      <Text fontWeight="600" color="blue.800">Important Note</Text>
                      <Text fontSize="sm" color="blue.700">
                        This request will be sent to the admin for approval. Your sponsor will only be changed after admin approval.
                      </Text>
                    </VStack>
                  </Alert>
                </VStack>
              ...
              <ModalFooter>
                <ButtonGroup spacing={4}>
                  <Button 
                    variant="ghost" 
                    onClick={onSponsorRequestModalClose}
                  >
                    Cancel
                  </Button>
                  <Button 
